...
<script>
    layui.use(['laydate', 'element', 'laypage', 'layer', 'form', 'table'], function () {
        ...
        function insertAdminUser(name,pass,roleId){
            $.ajax({
                "url" : "/adminList/insertBackUser"
                ,"data" : {"zhanghao" : name,"password":pass,"roleId1":roleId}
                ,"dataType" : "json"
                ,"success" :function (data) {
                    alert(data)
                }
        ...
        form.on('submit(save)', function(data){
            ...
            insertAdminUser(name,pass,role);
